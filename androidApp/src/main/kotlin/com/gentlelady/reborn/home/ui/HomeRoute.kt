...
fun HomeRoute() {
    ...
    val profileState by profileViewModel.state.collectAsState() // 🆕 프로필 상태 수집 추가
    ...
    LaunchedEffect(Unit) {
        ...
        profileViewModel.handleIntent(ProfileIntent.LoadProfile)
    }
    ...
    App(
        ...
        profileState = profileState, // 🆕 수집된 프로필 상태 파라미터 하향 주입
        onProfileIntent = { intent -> profileViewModel.handleIntent(intent) } // 🆕 프로필 인텐트 핸들러 바인딩 완료
    )
}